...
const CommentSection: React.FC<CommentSectionProps> = ({ articleId }) => {
  ...
  const organizeComments = (flatComments: CommentWithAuthor[]): CommentWithAuthor[] => {
  ...
  // 查找被回复的用户信息（用于深层回复）
  const findReplyTarget = (comment: CommentWithAuthor, allComments: CommentWithAuthor[]): { id: number; username: string; avatar?: string } | null => {
    if (!comment.parent_id) return null;
    
    const findCommentById = (comments: CommentWithAuthor[], id: string): CommentWithAuthor | null => {
  ...
  const renderComment = (comment: CommentWithAuthor, isReply = false) => (
    <div key={comment.id} className={`${isReply ? 'ml-6 mt-4 pl-4 border-l-2 border-blue-100 bg-gray-50/30' : 'border-b border-gray-100 last:border-b-0 px-4'} py-4 ${isReply ? 'rounded-r-lg' : ''}`}>
      <div className="flex items-start space-x-3">
        {/* 头像 */}
        <img 
          src={comment.author.avatar || '/default-avatar.svg'} 
          alt={comment.author.username}
          className={`${isReply ? 'w-7 h-7' : 'w-8 h-8'} rounded-full flex-shrink-0 object-cover ring-2 ring-white shadow-sm`}
        />
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* 评论文本 */}
          <p className={`text-gray-800 ${isReply ? 'text-xs' : 'text-sm'} leading-relaxed mb-3 text-left`}>
            {comment.content}
          </p>
          ...
          {replyingTo === comment.id && (
            <div className="mt-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
              <div className="mb-2">
                ...
                <span className="text-xs font-medium text-blue-600">@{comment.author.username}</span>
              </div>
              <textarea
                value={replyContent}
          ...
          {comment.replies && comment.replies.length > 0 && (
            <div className="mt-4 space-y-0">
              {(() => {
                // 递归收集所有回复，扁平化显示
                const collectAllReplies = (replies: CommentWithAuthor[]): CommentWithAuthor[] => {
                ...
                return allReplies.map(reply => (
                  <div key={reply.id} className="ml-6 mt-4 pl-4 border-l-2 border-blue-100 bg-gray-50/30 py-4 rounded-r-lg">
                    <div className="flex items-start space-x-3">
                      {/* 头像 */}
                      <img 
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          {/* 回复目标显示 - 对于回复显示被回复的用户 */}
                          {reply.parent_id && (() => {
                            const replyTarget = findReplyTarget(reply, comments);
                            return replyTarget ? (
                              <>
                                <span className="text-xs text-gray-400">回复</span>
                                <span className="text-xs font-medium text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">
                        ...
                        {replyingTo === reply.id && (
                          <div className="mt-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
                            <div className="mb-2">
                              <span className="text-xs text-gray-500">回复 </span>
                              <span className="text-xs font-medium text-blue-600">@{reply.author.username}</span>
                            </div>
                            <textarea
                              value={replyContent}
                              onChange={(e) => setReplyContent(e.target.value)}
    ...
    >
      <div className="mb-12">
        <div className="text-center mb-8">
          <h3 className="text-pink-500 font-semibold text-lg uppercase tracking-wider mb-2">COMMENTS</h3>
          <p className="text-gray-600 text-sm">
            {loading && comments.length === 0 ? '正在加载评论...' : `${totalCount} 条评论`}
          </p>
        </div>
      </div>
        ...
        {loading && comments.length === 0 ? (
          <div className="p-4">
            <div className="animate-pulse space-y-4">
              {[...Array(3)].map((_, index) => (
                <div key={index} className="flex items-start space-x-3 py-4 border-b border-gray-100 last:border-b-0">
