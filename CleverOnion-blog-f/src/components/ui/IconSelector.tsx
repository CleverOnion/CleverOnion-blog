...
import { getIconsByCategory, renderIcon, getAvailableIcons } from '../../utils/iconUtils';
...
const IconSelector: React.FC<IconSelectorProps> = ({
  ...
  const getDisplayIcons = () => {
    ...
    if (selectedCategory === 'all') {
      ...
      icons = iconsByCategory[selectedCategory as keyof typeof iconsByCategory] || [];
    }
    ...
    if (searchTerm) {
      ...
      icons = icons.filter(icon => {
        ...
        const withoutPrefix = iconLower.replace(/^(fa|md|si|fi|bs|ai|di|go|gr|hi|im|io|ri|tb|ti|vsc|wi)/, '');
        ...
        for (const [key, values] of Object.entries(keywords)) {
          if (values.some(v => searchLower.includes(v)) && iconLower.includes(key)) {
            return true;
          }
        }
             ...
             {!searchTerm && (
               <div className="mt-3">
                 ...
                 <div className="flex flex-wrap gap-2">
                   {['react', 'vue', 'node', 'css', 'mobile', 'database', 'git', 'design'].map((suggestion) => (
                     <button
                       key={suggestion}
                       type="button"
            ...
            {displayIcons.length > 0 ? (
              <>
                ...
                <div className="mb-4 text-sm text-gray-600">
                  {searchTerm ? (
                    <span>搜索 "{searchTerm}" 找到 {displayIcons.length} 个图标</span>
                  ) : (
                    <span>{categoryLabels[selectedCategory as keyof typeof categoryLabels]} - {displayIcons.length} 个图标</span>
                  )}
                </div>
                  ...
                  {displayIcons.map((iconName) => (
                    ...
                    >
                      {renderIcon(iconName, `w-6 h-6 ${
                        value === iconName ? 'text-blue-600' : 'text-gray-600 group-hover:text-gray-800'
                      }`)}
              ...
              <div className="flex flex-col items-center justify-center py-16 text-gray-500">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">没有找到匹配的图标</h3>
