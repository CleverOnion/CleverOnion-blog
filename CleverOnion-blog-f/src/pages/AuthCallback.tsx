  ...
  useEffect(() => {
    const handleAuthCallback = async () => {
      try {
        ...
        // 处理GitHub回调
        setMessage('正在验证授权信息...');
        const loginData = await GitHubOAuth.handleCallback(code, state || undefined);
  ...
  return (
    <div className="min-h-screen relative bg-gradient-to-b from-sky-300 to-white overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center px-4">
          ...
          >
            ...
            {status === 'loading' && (
              ...
              >
                ...
                <p className="text-gray-600 font-medium" style={{fontFamily: 'Comic Sans MS, cursive, sans-serif'}}>{message}</p>
              </motion.div>
            )}
        ...
        >
          <p className="text-sm text-gray-600 bg-white/70 backdrop-blur-sm rounded-lg px-4 py-2 inline-block border border-gray-300" style={{fontFamily: 'Comic Sans MS, cursive, sans-serif'}}>
            如果页面长时间无响应，请刷新页面重试
          </p>
        </motion.div>
      </motion.div>
